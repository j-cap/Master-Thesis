...
As second example, we will now investigate the behavior of a servo-compensation. \pref{fig:blockschaltbild} shows the block diagram of the control concept including the servo-compensation. The region of interest is marked by the red square. Here we aim to find a model which approximates the static non-linear mapping of two inputs to the differential current $i_m^d$. The first input is the measured position of the main valve $s_h$. The second input is the differential flow $q_p^d$. 
