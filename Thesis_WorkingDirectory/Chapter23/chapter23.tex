...
We are now going to use the theory discussed in Chapter \ref{chap:fundamentals} to estimate uni- and bivariate  functions using data and a priori domain knowledge. An overview of the different problems considered in this work is given in Table \ref{tab:problem_overview}. 
...
\end{align}
%
where $\text{pen}(\vec{\beta})$ specifies a smoothness penalty term. Finally, we are going to incorporate a priori domain knowledge into the fitting process using the approach given by Hofner in~\cite{hofner2011monotonicity} and apply it to uni- and bivariate functions, i.e. we solve the optimization problem
\begin{align}
...
\end{align}
%
where $\text{con}({\vec{\beta}})$ specifies the user-defined constraint to incorporate a priori domain knowledge with. Various types a priori domain knowledge can be incorporated using the constraints listed in Table~\ref{tab:constraint_overview}.
...
\begin{table}[h]
	...
	\begin{tabular}{|l|ll|l|}
		...
		\multirow{2}{*}{Boundedness}  & lower & $\lvert f(x)\rvert \ge M$ 	  &	12 \\ \cline{2-4}
		& upper& $\lvert f(x)\rvert \ge M$    & 12 \\ \hline
...
\end{align}
%
generated by random sampling of $x \in [0,1]$ of the univariate function $f_1(x)$ given by 

\begin{align}\label{eq:test_func}
	y^{(i)} = f_1(x^{(i)}) + \epsilon_i = 3\sin(3\pi x) + 17x + 3 + \epsilon_i.
\end{align}
%
where $\epsilon_i$ is Gaussian noise given by $\mathcal{N}(0, 0.1)$. The data is shown in~\pref{fig:test_func}. 



The function is partially increasing.  The test function for the two dimensional case is given in Chapter \ref{subsec:2d-example}. 
